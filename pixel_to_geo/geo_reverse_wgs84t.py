...
def pixel_to_latlon(image_path, u, v,
    ...
    R, t = build_extrinsics(lat, lon, alt, roll, pitch, yaw,
                            zone, off_e, off_n)
    # back-project pixel to local UTM‐relative XYZ
    x_l, y_l, _ = pixel_to_local_xyz(u, v, K, dist, R, t)
    # global UTM coordinates
    x_utm, y_utm = off_e + x_l, off_n + y_l
    # convert to lat/lon
    lat_pt, lon_pt = utm_to_latlon(x_utm, y_utm, zone)
    return lat_pt, lon_pt, alt
...
if __name__ == "__main__":
    ...
    lat, lon, alt = pixel_to_latlon(
        ...
        camera_json=r"nodeodm_out_4\cameras.json",
        coords_txt=r"nodeodm_out_4\odm_georeferencing\coords.txt"
    )
    print(
